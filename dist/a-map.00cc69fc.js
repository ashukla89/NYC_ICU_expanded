...
var projection = d3.geoMercator().center([-73.94, 40.70]).scale(45000).translate([width / 2, height / 2]);
...
function ready(_ref) {
  ...
  svg.selectAll('path').data(boroughs).enter().append('path').attr('class', 'borough').attr('d', path).attr('fill', 'lightgrey').attr('stroke', 'grey');
  svg.append('text').text('NYC Hospitals by Percent of ICU Beds Filled').attr('x', width / 2).attr('y', 0).attr('text-anchor', 'middle').attr('font-size', 18).attr('font-weight', 'bold'); // add one circle for every hospital
  ...
  .attr('transform', function (d) {
    ...
    d3.select('#hosp-head').text('Hospital Name').style('text-anchor', 'left').style('font-size', 12).style('font-weight', 'bold');
    d3.select('#hosp').text(d.hospital_name).style('text-anchor', 'left').style('font-size', 12);
    d3.select('#totalbeds-head').text('Avg. Total ICU Beds During Week').style('text-anchor', 'left').style('font-size', 12).style('font-weight', 'bold');
    d3.select('#totalbeds').text(d.total_icu_beds_7_day_avg).style('text-anchor', 'left').style('font-size', 12);
    d3.select('#occupied-head').text('Avg. ICU Beds Occupied During Week').style('text-anchor', 'left').style('font-size', 12).style('font-weight', 'bold');
    d3.select('#occupied').text(d.icu_beds_used_7_day_avg).style('text-anchor', 'left').style('font-size', 12);
    d3.select('#percent-head').text('Percent of ICU Beds Occupied').style('text-anchor', 'left').style('font-size', 12).style('font-weight', 'bold');
    d3.select('#percent').text(d3.format(".0%")(+d.icu_beds_used_pct_7_day_avg)).style('text-anchor', 'left').style('font-size', 12);
