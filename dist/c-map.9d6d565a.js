...
var colorScale = d3.scaleOrdinal(d3.schemeCategory10); // nabbed a nice one from: https://github.com/d3/d3-scale-chromatic
...
function ready(datapoints) {
  ...
  svg.append('text').attr('font-size', '24').attr('text-anchor', 'middle').text('Percent of Beds Filled, by Borough').attr('x', width / 2).attr('y', -40).attr('dx', 40).attr('class', 'title');
  ...
  svg.selectAll('circle').data(nested).enter().append('circle').attr('class', function (d) {
    ...
    return colorScale(d.key.toLowerCase().replace(/[^a-z]/g, ''));
  }).attr('r', 4).attr('cx', width).attr('cy', function (d) {
    var datapoints = d.values; // Find the last datapoint, as per https://flaviocopes.com/how-to-get-last-item-array-javascript/

    var lastWeek = datapoints[datapoints.length - 1];
  ...
  }).classed('labels', true).attr('y', function (d) {
    // again, return y pos of last item 
    return yPositionScale(d.values[d.values.length - 1].icu_beds_used_pct_7_day_avg);
  }).attr('x', width).text(function (d) {
    return d.key;
  }).attr('dx', 6).attr('dy', 4).attr('font-size', '12'); // here are the axes

  var xAxis = d3.axisBottom(xPositionScale).tickFormat(d3.timeFormat('%b %d, %Y')).ticks(9);
  svg.append('g').attr('class', 'axis x-axis').attr('transform', 'translate(0,' + height + ')').call(xAxis);
  var yAxis = d3.axisLeft(yPositionScale).tickFormat(d3.format(".0%"));
